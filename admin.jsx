...
function AdminClearances({ clubs, players, clearanceRequests, onAdminOverride, toast }) {
  ...
  const list = filter === "overdue"  ? overdue
    ...
    <div>
      <div className="page-head">
        <div className="ph-left">
          ...
          <h1 className="ph-title">Player <em>Clearances</em></h1>
          <p className="ph-desc">
            Every clearance request across the cohort. Source clubs have 14 days to confirm fees + misconduct.
            After that window, the Lions office can override and approve on the source club's behalf.
          </p>
        </div>
        <div className="ph-actions">
          <Btn tone="outline" size="sm" icon={Icon.Download}>Export</Btn>
          <Btn tone="outline" size="sm" icon={Icon.Mail}>Remind overdue clubs</Btn>
        </div>
      </div>
      ...
      <div className="players-stats">
        ...
        <div className="players-stat">
          ...
          <div className="players-stat-n" style={{color:"var(--gold)"}}>{pending.length}</div>
        </div>
        <div className="players-stat">
          <div className="players-stat-l">Overdue (&gt; 14 days)</div>
          <div className="players-stat-n" style={{color:"var(--coral)"}}>{overdue.length}</div>
        </div>
        <div className="players-stat">
          <div className="players-stat-l">Resolved</div>
          <div className="players-stat-n" style={{color:"var(--green)"}}>{resolved.length}</div>
        </div>
      </div>
      ...
      <div className="filter-row" style={{marginTop:14}}>
        {[
          ...
          <button key={b.k} className={`filter-pill ${filter===b.k?"active":""}`} onClick={()=>setFilter(b.k)}>
      ...
      <div className="clr-list" style={{marginTop:14}}>
        {list.length === 0 && (
          <div style={{padding:"40px 16px", textAlign:"center", color:"var(--muted)", fontSize:13, background:"var(--white)", border:"1px solid var(--line)", borderRadius:"var(--radius-lg)"}}>
        ...
        {list.map(req => {
          ...
          return (
            <div key={req.id} className={`clr-card admin ${overdue?"overdue":""} ${req.status!=="pending"?"resolved":""}`}>
              <div className="clr-card-head">
                <div>
                  <div className="clr-eyebrow">
                    {req.status === "admin-override" ? "✓ Lions override" :
                     req.status === "approved"        ? `✓ Cleared by ${fromClub?.short || fromClub?.name}` :
                     overdue                          ? `⚠ Overdue · ${elapsed} days` :
                                                       `Pending · ${daysLeft} days remaining`}
                  </div>
                  <div className="clr-name">{player?.firstNames} {player?.surname}</div>
                  <div className="clr-meta">
                    ID {player?.idNumber} · {player?.team} · Requested {new Date(req.requestedAt).toLocaleDateString("en-GB",{day:"numeric",month:"short"})}
                  </div>
                </div>
              ...
              {overdue && req.status === "pending" && (
                <div className="clr-override">
                  <div className="clr-override-text">
                    <div className="clr-override-title">{fromClub?.name} hasn't actioned this in {elapsed} days.</div>
                    <div className="clr-override-sub">
                      The Lions office can override the source club's approval and issue the clearance directly to {toClub?.name}.
                    </div>
                  </div>
                  <Btn tone="teal" icon={Icon.Arrow} onClick={()=>setConfirm({
                    ...
                    onYes: () => { onAdminOverride(req.id); setConfirm(null); toast?.(`${player?.firstNames} ${player?.surname} cleared to ${toClub?.short || toClub?.name} · Lions override`); },
              ...
              {req.status !== "pending" && (
                <div className="clr-resolved-bar">
                  ...
                  <span style={{fontSize:11,color:"var(--muted)",fontFamily:"'Montserrat',sans-serif"}}>
                    {new Date(req.clubApprovedAt || req.adminOverrideAt).toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"})}
                  </span>
                </div>
              )}
            </div>
      ...
      {confirm && ReactDOM.createPortal(
        <div className="fix-confirm" onClick={(e)=>e.target===e.currentTarget && setConfirm(null)}>
          <div className="fix-confirm-box">
            ...
            <div className="fix-confirm-actions">
              <Btn tone="outline" onClick={()=>setConfirm(null)}>Cancel</Btn>
              <Btn tone="teal" icon={Icon.Arrow} onClick={confirm.onYes}>Yes, override</Btn>
            </div>
          </div>
        </div>,
        document.body
...
Object.assign(window, {
  AdminDashboard, AdminClubsList, AdminClubDetail, AdminFixtures, CreateSeriesForm,
  AdminClearances,
});
